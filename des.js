...
const hexToBin = (hex) =>
  ("00000000" + parseInt(hex, 16).toString(2)).slice(-8);
...
// encodes a message using DES, where msg and key are binary, and returns binary
export const encode = (msg, key) => des(msg, keySchedule(key));

export const initialHexToBin = (key) =>
  chunkString(key, 2)
    .map((hex) => hexToBin(hex))
    .join("");

export const finalBinToHex = (bin) =>
  chunkString(bin, 4).map(binToHex).join("");
